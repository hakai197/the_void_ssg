...
export default function GooeyNav({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty('--color', `var(--color-${p.color}, #8b2fc9)`);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement | HTMLAnchorElement>, index: number) => {
          ...
          {items.map((item, index) => (
            ...
            >
              {item.href ? (
                ...
                <button type="button" onKeyDown={(e) => handleKeyDown(e, index)}>
                  {item.label}
                </button>
              )}
            </li>
